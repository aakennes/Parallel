...
inline u32 mul_mod_harvey_lazy(const u32 modulus, const u32 in1, const u32 in2,
                               const u32 in2_harvey) {
    u32 approx_quotient = (u64)in1 * in2_harvey >> 32;
    return (u64)in1 * in2 - (u64)approx_quotient * modulus;
}
namespace hehub {
    ...
    NTTFactors(u32 modulus, size_t log_dimension, bool for_inverse = false) {
        ...
        if (!for_inverse) {
            ...
            for (size_t i = 0; i < dimension; i++) {
                ...
                seq_harvey[i] = ((u64)seq[i] << 32) / modulus;
            }
        } else {
            ...
            for (size_t l = 0; l < log_dimension; l++) {
                ...
                for (size_t i = 0; i < (1 << l); i++) {
                    ...
                    seq_harvey[idx] = ((u64)seq[idx] << 32) / modulus;
                }
            }
            const u32 dimension_inv = modulus - ((modulus - 1) >> log_dimension);
            const u32 dimension_inv_harvey =
                ((u64)dimension_inv << 32) / modulus;
            for (size_t i = 0; i < dimension; i++) {
...
inline const auto &
__find_or_create_ntt_factors(const u32 modulus, const size_t log_dimension,
...
void ntt_negacyclic_inplace_lazy(const size_t log_dimension, const u32 modulus,
                                 u32 coeffs[]) {
...
void intt_negacyclic_inplace_lazy(const size_t log_dimension, const u32 modulus,
                                  u32 values[]) {
    const size_t dimension = 1ULL << log_dimension;
    // generate or read from cache
...
void cache_ntt_factors_strict(const u32 log_dimension,
                              const std::vector<u32> &moduli) {
